...
elemental function curvature(phi)
	...
	if (mu > 0.0 .and. phi > 0.0) then
		...
		x = ((-(n*alpha/beta**2)/(phi-2.0*upsilon))**(n/(n+1.0)) - 1.0/beta)**(1.0/n)
	end if
	
	! Newton's method
	do i = 1,16
		p = beta*x**n + 1.0; q = (n*alpha)*x**(n-2); s = p*p
		x = x + ((phi - 2.0*mu*x)*s + q*x)/(2.0*mu*s + q*(n+1-2*n/p))
	end do
...
elemental function DDV(phi)
        ...
        x = curvature(phi); p = beta*x**n + 1.0; q = (n*alpha)*x**(n-2); s = p*p
        DDV = (R0/3.0) * (s - q*x*(n+2-2*n/p))/(2.0*mu*s + q*(n+1-2*n/p))
end function DDV
